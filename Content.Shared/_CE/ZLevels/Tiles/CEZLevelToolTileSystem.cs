...
public sealed partial class CEZLevelToolTileSystem : EntitySystem
{
    ...
    private void UseToolOnTileAbove(Entity<CEZLevelToolTileComponent> ent, EntityUid user, EntityCoordinates clickLocation)
    {
        ...
        if (GetMatchingQuality(ent.Owner, tool, tileDef.DeconstructTools) is not { } requiredQuality)
        {
        ...
        _tool.UseTool(ent, user, ent, tileComp.Delay, new[] { requiredQuality }, doAfterArgs, out _, toolComponent: tool);
    }
    ...
    private void OnToolTileComplete(Entity<CEZLevelToolTileComponent> ent, ref CEZLevelTileToolDoAfterEvent args)
    {
        ...
        var tileDef = (ContentTileDefinition)_tileDefManager[tileRef.Tile.TypeId];
        if (GetMatchingQuality(ent.Owner, tool, tileDef.DeconstructTools) is null)
            return;
    ...
    /// <summary>
    /// Returns the first quality the tool has that the tile also accepts, or null when the tool
    /// cannot deconstruct this tile at all. Tool qualities are read through <see cref="SharedToolSystem"/>
    /// because <see cref="ToolComponent"/> only allows access from there.
    /// </summary>
    private ProtoId<ToolQualityPrototype>? GetMatchingQuality(EntityUid toolUid, ToolComponent tool, HashSet<ProtoId<ToolQualityPrototype>> accepted)
    {
        foreach (var quality in accepted)
        {
            if (_tool.HasQuality(toolUid, quality, tool))
                return quality;
        }
